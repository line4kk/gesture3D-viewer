...
| Указательные пальцы обеих рук направлены вниз, как жест "здесь", положение удерживается около секунды | Сброс положения камеры | `reset_view` |
| Две руки в форме буквы L, сложенные как рамка кадра, положение удерживается около секунды | Действие для скриншота. Сейчас это временная заглушка во frontend | `screenshot` |

Приоритет жестов задается в backend: если несколько detector-классов потенциально подходят под текущий кадр, обработчик выбирает первое распознанное событие по порядку подключения detector'ов.
